// content.js - Content script with embedded parser logic
...
class LinkedDataParser extends ProductDataParser {
  constructor() {
    super();
  }

  // Get current page product data using all available parsing methods
  getCurrentProductData() {
    if (!this.productData) {
      this.productData = this.parseAllSources(document);
    }
...
// Initialize parser
const parser = new LinkedDataParser();

// Listen for messages from popup
browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "getProductData") {
    const productData = parser.getCurrentProductData();
